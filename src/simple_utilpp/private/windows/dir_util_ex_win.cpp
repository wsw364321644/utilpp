...
namespace utilpp {
    namespace DirUtilEx {
        ...
        bool GetAccessControlForUserInternal(PSECURITY_DESCRIPTOR psd, EUserType userType, uint64_t* mask) {
        ...
        bool GetAccessControlForUser(std::u8string_view  path, EUserType userType, uint64_t* mask, std::error_code& ec) {
            PathBuf.SetPath((char*)path.data(), path.size());
            return GetAccessControlForUser(PathBuf, userType, mask, ec);
        }
        bool GetAccessControlForUser(FPathBuf& pathBuf, EUserType userType, uint64_t* mask, std::error_code& ec)
        {
            DWORD                dw;
            PACL                 pacl{ 0 };
            PSECURITY_DESCRIPTOR psd;
        ...
        //    }
        //}
            bool bres = GetAccessControlForUserInternal(psd, userType, mask);
            LocalFree(psd);
            return bres;
        }
        ...
        bool GrantAccessControlForUser(std::u8string_view  path, EUserType userType, uint64_t mask, std::error_code& ec) {
            PathBuf.SetPath((char*)path.data(), path.size());
            return GrantAccessControlForUser(PathBuf, userType, mask, ec);
        }
        ...
        {
            PathBuf.SetPath((char*)path.data(), path.size());
            return GetVersionInfo(PathBuf, info, ec);
        }
        bool GetVersionInfo(FPathBuf& pathBuf, FileVersionInfo_t& info, std::error_code& ec)
        {
            ...
            DWORD dwSize = 0;
            dwSize = GetFileVersionInfoSizeW(pathw, NULL);
            if (dwSize == 0) {
                ec = std::error_code(GetLastError(), std::system_category());
                return false;
                //if (ERROR_RESOURCE_TYPE_NOT_FOUND == ) {
                //    break;
                //}
                //else {
                //    ec = std::error_code(dwRes, std::system_category());
                //    printf( "Error in GetFileVersionInfoSize: %i\n", GetLastError());
                //    return sonkwo_error_number::ERR_UNKNOWN;;
                //}
            ...
            if (!VerQueryValueW(buf.Data(), L"\\", (LPVOID*)&pFileInfoLocal, &puLenFileInfoLocal))
            {
                ec = std::error_code(GetLastError(), std::system_category());
                return false;
            }
            info.FileVersion = ((uint64_t)pFileInfoLocal->dwFileVersionMS << 32) | pFileInfoLocal->dwFileVersionLS;
            info.ProductVersion = ((uint64_t)pFileInfoLocal->dwProductVersionMS << 32) | pFileInfoLocal->dwProductVersionLS;
